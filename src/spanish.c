...
Embedding es_embeddings[MAX_WORDS];  // Changed from fr_embeddings to es_embeddings
TranslationPair test_pairs[MAX_TEST_PAIRS];
int en_count = 0;
int es_count = 0;  // Changed from fr_count to es_count
int test_count = 0;
...
void find_top_k(const float* source_vector, int k, int* top_indices, float* top_scores) {
    ...
    for (int i = 0; i < es_count; i++) {  // Changed from fr_count to es_count
        float sim = cosine_similarity(source_vector, es_embeddings[i].vector);  // Changed from fr_embeddings to es_embeddings
...
const char* translate_word(const char* english_word, int top_k, int* top_indices, float* top_scores) {
    ...
    // === Step 4: Find Top K most similar Spanish words ===
    ...
    for (int i = 0; i < top_k && top_indices[i] >= 0; i++) {
        if (strcmp(es_embeddings[top_indices[i]].word, cleaned) != 0 &&  // Changed from fr_embeddings to es_embeddings
            top_scores[i] >= MIN_SIMILARITY) {
            return es_embeddings[top_indices[i]].word;  // Changed from fr_embeddings to es_embeddings
        }
    }

    // === Step 6: If fallback needed, return best available word ===
    if (top_indices[0] >= 0) {
        if (strcmp(es_embeddings[top_indices[0]].word, cleaned) == 0) {  // Changed from fr_embeddings to es_embeddings
            return "<same>";
        }
        return es_embeddings[top_indices[0]].word;  // Changed from fr_embeddings to es_embeddings
    }
...
int load_test_pairs(const char* filename) {
    ...
    while (fgets(line, sizeof(line), file) && count < MAX_TEST_PAIRS) {
        ...
        char* es_word = strtok(NULL, "\t\n ");  // Changed from fr_word to es_word
        
        if (en_word && es_word) {  // Changed from fr_word to es_word
            strncpy(test_pairs[count].source, en_word, MAX_WORD_LEN - 1);
            strncpy(test_pairs[count].target, es_word, MAX_WORD_LEN - 1);  // Changed from fr_word to es_word
...
void evaluate_accuracy() {
    ...
    for (int i = 0; i < test_count; i++) {
        ...
        for (int j = 0; j < TOP_K && top_indices[j] >= 0; j++) {
            if (strcmp(es_embeddings[top_indices[j]].word, test_pairs[i].target) == 0) {  // Changed from fr_embeddings to es_embeddings
                correct_top5++;
                break;
            }
        }
...
int main() {
    ...
    printf("Loading Spanish embeddings...\n");  // Changed from French to Spanish
    es_count = load_embeddings("wiki.es.align.vec", es_embeddings);  // Changed filename and variable
    printf("Loaded %d Spanish words.\n", es_count);

    if (en_count == 0 || es_count == 0) {  // Changed from fr_count to es_count
    ...
    while (1) {
        ...
        for (int i = 0; i < TOP_K && top_indices[i] >= 0; i++) {
            printf("%d. %s (score: %.4f)", i+1, es_embeddings[top_indices[i]].word, top_scores[i]);  // Changed from fr_embeddings to es_embeddings
            if (strcmp(es_embeddings[top_indices[i]].word, input) == 0) {  // Changed from fr_embeddings to es_embeddings
