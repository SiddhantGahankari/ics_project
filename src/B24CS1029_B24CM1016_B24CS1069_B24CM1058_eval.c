...
void evaluate_accuracy_fr()
{
    ...
    {
         char *translation = translate_word_fr(test_pairs[i].source, TOP_K, top_indices, top_scores);
...
void evaluate_accuracy_es(){
    ...
    for (int i = 0; i < test_count; i++)
    {
         char *translation = translate_word_es(test_pairs[i].source, TOP_K, top_indices, top_scores);
